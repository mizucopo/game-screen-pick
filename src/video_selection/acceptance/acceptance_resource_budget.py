...
def acceptance_run_resource_budget_passed(record: Mapping[str, object]) -> bool:
...
def _integer(value: Mapping[str, object], key: str) -> int:
    ...
    if not isinstance(result, int) or isinstance(result, bool):
        raise ValueError(f"Acceptance run metric {key}がintegerではありません")
    return result
...
def _boolean(value: Mapping[str, object], key: str) -> bool:
    ...
    if not isinstance(result, bool):
        raise ValueError(f"Acceptance run metric {key}がbooleanではありません")
    return result
