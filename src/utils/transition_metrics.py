...
def calculate_bright_washout_score(raw_metrics: "RawMetrics") -> float:
    ...
    edge_penalty = 1.0 - clamp01(raw_metrics.edge_density / BRIGHT_WASHOUT_EDGE_SCALE)
    range_penalty = 1.0 - clamp01(
        raw_metrics.luminance_range / BRIGHT_WASHOUT_RANGE_SCALE
    )
    return clamp01(
...
def calculate_support_ui_score(normalized_metrics: "NormalizedMetrics") -> float:
    ...
    return clamp01(
        ...
        + SUPPORT_UI_ACTION_INVERSE_WEIGHT * (1.0 - normalized_metrics.action_intensity)
    )
...
def calculate_veiled_transition_score(
    ...
    return clamp01(
        ...
        + VEILED_INFORMATION_INVERSE_WEIGHT * (1.0 - adaptive_scores.information_score)
