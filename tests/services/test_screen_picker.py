...
class TestSelectFromAnalyzed:
    """Test suite for the static select_from_analyzed method.

    This method contains pure domain logic without any IO operations,
    making it ideal for thorough testing without mocks.
    """

    def test_high_scoring_images_are_selected_first_respecting_diversity_constraint(  # noqa: E501
        self, sample_image_metrics
    ):
    ...
    def test_requesting_more_images_than_available_returns_all_available(
        self, sample_image_metrics
    ):
    ...
    def test_similarity_threshold_affects_diversity_of_selection(
        self, sample_image_metrics
    ):
    ...
    def test_strict_similarity_threshold_filters_out_similar_images(
        self, sample_image_metrics
    ):
    ...
    def test_selecting_zero_images_returns_empty_list(self, sample_image_metrics):
    ...
    def test_does_not_modify_original_input_list(self, sample_image_metrics):
...
class TestGameScreenPickerInit:
    """Test suite for GameScreenPicker initialization."""

    def test_picker_stores_analyzer_reference(self, mock_analyzer):
...
class TestLoadImageFiles:
    """Test suite for _load_image_files method.

    These tests use real filesystem operations (temp directories)
    instead of mocks, as filesystem is a managed dependency in tests.
    """

    def test_finds_supported_image_formats_in_folder(self):
    ...
    def test_recursive_search_finds_images_in_subdirectories(self):
    ...
    def test_non_recursive_search_only_checks_top_level(self):
...
class TestSelectIntegration:
    """Integration tests for the select method.

    These tests test the full flow: file loading -> analysis -> selection.
    Uses real filesystem but mocked analyzer to avoid heavy ML operations.
    """

    def test_select_from_folder_returns_diverse_high_quality_images(
        self, mock_analyzer
    ):
        ...
        Then:
            - Returns 3 diverse, high-quality images
        """
        # Arrange
        with tempfile.TemporaryDirectory() as temp_dir:
            # Create test images
    ...
    def test_select_skips_files_that_analyze_returns_none(self, mock_analyzer):
    ...
    def test_select_from_nonexistent_folder_returns_empty_list(self, mock_analyzer):
