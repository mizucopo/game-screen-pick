...
5. 戦闘可視性専用確認: 戦闘有無専用確認で戦闘と確認された場合、または同確認の対象になった`recurring_gameplay`のactionに実行します。同じRepresentative Frame一枚だけを入力し、音声、Context Cue、前後場面、主推論の説明文を渡しません。見下ろし型・遠景・非人型の小型敵は、gameplay spriteの輪郭全体が判別できれば`clear`かつ`complete`とします。さらに攻撃相手本体の見せ方を`prominent`・`recognizable`・`weak`・`absent`、playerと相手の戦闘関係を`direct`・`indirect`・`none`で観測します。複数敵では最も明瞭かつ完全に収まる一体を基準とし、別の敵が画面端にいることやplayer付近のeffectだけでは掲載価値を下げません。敵が主要な被写体として識別できる場合、または小さくても単体で識別できて直接的な戦闘関係が見える場合だけ掲載可能です。
6. 戦闘可視性の独立再確認: 戦闘と確認済みで最初の戦闘可視性専用確認が掲載可能を返した場合、または二回の戦闘有無確認がともに非戦闘だった場合に、先の回答を参照しない別promptで同じ画素を観測し直します。前者は二回とも敵本体が明瞭で構図内に収まり、かつ上記のOpponent PresentationとCombat Interaction Visibilityの境界を満たす場合だけ掲載価値を保持します。後者は二回とも敵本体が不在でエフェクトだけではない場合、または二回とも掲載可能な戦闘として一致する場合だけ掲載価値を保持します。
...
- 画像内の攻撃相手一体だけから観測したCombat Subject Evidence
...
Combat Subject Evidenceを持たない`game-screen-pick/candidate-annotation@4.0.0`と、現行の掲載可読性を持たないCandidate Annotationは現行contractとして復元せず、`candidate-annotation-prompt-v18`、`candidate-annotation-schema-v13`、`candidate-annotation-stage-v36`で該当Annotationとdownstreamだけを再計算します。戦闘可視性のatomic contractは`combat-visibility-verification-prompt-v4`、`combat-visibility-verification-schema-v3`、`combat-visibility-verification-stage-v4`です。Video Identity、Video Stage、Context Cue、無関係なmodel Stageは再利用し、動画のwhole-file SHA-256やframe抽出をやり直しません。現行artifact schemaは`game-screen-pick/candidate-annotation@5.0.0`です。
