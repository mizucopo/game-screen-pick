...
response/schema/domain validation retryではstable validation codeを追加し、raw responseを次promptへ戻しません。戦闘可視性の相関違反では、攻撃相手本体が不在なら構図・見せ方も不在で戦闘関係は`none`、本体が存在するなら構図・見せ方を不在にしない、player本体が不在なら戦闘関係を`direct`にしない、という既存validatorの関係をretry promptへ明示します。strict validatorを緩和しません。再試行後もplayer本体不在と`direct`だけが矛盾する場合は、二回目の独立観測を保持し、従属するCombat Interaction Visibilityだけを`none`へ安全側に修復して全fieldを再検証します。stable validation codeをdiagnosticsへ残し、掲載価値を上げずCandidate Annotationを完了します。その他の相関違反が再試行後も残る場合はfatalにします。Scene Kind `other`のsceneは自由なslugが返っても、分類の逃げ先として正確な`other`、汎用表示名と汎用説明へ決定的に正規化します。Scene Catalogのdomain違反ではScene Kindの重複を許しつつScene Slugを一意にし、`other`のkind・role関係を満たす修正指示を再提示します。再試行でも非`other`のScene Slugだけが重複した場合は、入力順に`-2`、`-3`のsuffixを付けて決定的に一意化します。Scene Kind `other`が複数ある場合やその他のdomain違反は補正せず失敗します。Context Cueを持つCinematic Event Presentationまたは大きなevent portraitで画面内台詞文字ありと返された場合は、同じ画像とsemantic入力を使う主推論の一回のretryで、音声やContext Cueを根拠にせずDialogue Text Presentationを再確認します。再確認でも台詞文字ありなら有効な会話画面として保持し、文字表示なしなら静止eventへ正規化します。
...
Combat Subject Evidenceを持たない`game-screen-pick/candidate-annotation@4.0.0`と、現行の掲載可読性を持たないCandidate Annotationは現行contractとして復元せず、`candidate-annotation-prompt-v18`、`candidate-annotation-schema-v13`、`candidate-annotation-stage-v38`で該当Annotationとdownstreamだけを再計算します。戦闘可視性のatomic contractは`combat-visibility-verification-prompt-v5`、独立再確認は`combat-visibility-confirmation-prompt-v4`、共通schemaは`combat-visibility-verification-schema-v3`、verification Stageは`combat-visibility-verification-stage-v4`です。Video Identity、Video Stage、Context Cue、無関係なmodel Stageは再利用し、動画のwhole-file SHA-256やframe抽出をやり直しません。現行artifact schemaは`game-screen-pick/candidate-annotation@5.0.0`です。
