...
Video Set内の全sourceをVideo Order順にprobeした後、独立した`scan-video`をbounded並列実行します。既定の`video_scan.workers = "auto"`では、CPU decodeはprocessに許可されたlogical CPU 8個につき1 workerの保守的上限を使います。このCPU数はprocess affinityを起点に、processへ適用されるcgroup v2またはv1の各ancestor CPU quotaで制限します。NVDECはCPU・memory・NVIDIA Decoder・GPU・VRAM・diskの初期sampleにpressureがあれば1 worker、正常時は同じ保守値から開始します。その後は各resourceと1 streamあたりの処理速度をrolling windowで観測し、余力があればlogical CPU 4個につき1 worker、既定で最大6 workerまで使います。利用率は直近3 sample、disk throughputと処理速度のtrendは直近2 sample対その前の2 sampleで判断します。並列数はscan完了境界で1ずつ変更し、active scanを停止せず未開始taskの投入だけを調整します。resource sample、disk観測、またはstream速度trendを取得できない場合は増加せず、安全側のworker数を維持します。

Video Order上の対象scanが確定した時点で、後続Videoのscanを続けながら、そのVideoの`extract-frame-candidates`を確定し、続いて`collect-context`を開始します。downstream、結果、progress通知はVideo Order順であり、後続scanの完了順には依存しません。worker数と変更履歴はStage Fingerprintやcache identityへ含めず、privacy-safeなrun provenanceへだけ記録します。各Stage境界ではpath・size・`mtime_ns` snapshotを検査し、内容のwhole-file SHA-256はVideo Identity cache miss時だけ計算します。device、inode、ctimeは再利用判定に使いません。Video IdentityのSHA-256はdisk/CPU処理であり、NVDECへ移せません。

一つのVideo Sourceの`extract-frame-candidates`では、互いに離れたRefinement Window Groupをbounded並列実行します。worker数はGroup数、最大4、現在利用可能なmemory、Video Scanと共有するlogical CPU予算から決定します。CPU decodeのactive scanは1件あたり8 logical CPU、NVDEC scanは4 logical CPU、Refinementは1件あたり4 logical CPUを予約済みとして扱います。次に処理するsourceはscan完了待機前に最低1 Refinement worker分を要求するため、scan完了境界では後続scanの補充よりRefinementが優先されます。Refinement実行中は選択worker分をVideo Scan Schedulerへ予約し、その後Controllerが増員しても新しいscanの投入を残りCPU容量内へ制限します。active scanが全CPUを使用している場合は新規scanの補充を止め、1 Refinement worker分が空く完了境界まで待ちます。待機中にbackground scanが失敗した場合はRefinementを開始せず、そのscan失敗でrunを中止します。これはFFmpegのsoftware range decodeとOpenCV/NumPyによるmodel-free解析を行うCPU・disk処理であり、OllamaのGPU推論ではありません。worker数はpublic設定やsemantic identityに加えず、同じ入力を1 workerまたは4 workerで処理しても結果をPTS range順へ戻します。処理全体のresource対応は[Pipeline処理フローと計算資源](processing-flow.md)を参照してください。

Ctrl+Cでは未開始の`scan-video`を先に取り消し、その後で実行中のscanへ終了を要求します。Refinement Window Groupの並列処理中はqueued Groupを取り消し、実行中のFFmpeg range decoderへ終了を要求して回収してから終了します。割り込み後に待機中のscanやGroupを新しく開始しません。Completed Stageが未確定でも、atomicに確定済みのscan partition、Refinement Window Group、Embedded Subtitle stream、PCM sample range、STT chunkは次回runで再利用し、実行中または未開始だった最小Work Unitだけを再計算します。
...
2. `extract-frame-candidates`
   ...
   - Group数、Video Scanと共有するprocess許可済みlogical CPU、最大4のsafe cap、available memoryからworker数を自動決定します。logical CPUはprocess affinityを起点に、applicable cgroup v2 `cpu.max`またはcgroup v1 CFS quotaの各ancestor上限で制限し、小数quotaを安全側へ切り下げます。available memoryはsystem余力と、processに適用されるcgroup v2またはv1の各ancestor残量の最小値です。memory上限はVideo Scanで全native frameから観測した最大幅・最大高さを長辺960pxへ縮小し、保持frameあたり4 byte/pixel、最低240fpsの保守値と実測した最小PTS差・同一PTS最大frame数による上限の大きい方で各PTS rangeを見積もります。同時Groupはavailable memoryの4分の1へ収まる値とし、残る4分の3はOpenCV/NumPyの解析用一時領域、runtime、他processの余裕として予約します。一Groupもこのparallel予算へ収まらない場合は、従来と同じ逐次1 workerへ戻して並列化によるmemory増幅を防ぎます。旧cacheなどで完全なframe timing・寸法hintを取得できない場合、またはavailable memoryを取得できない場合も、cacheを失効させず1 workerへ抑制します。
