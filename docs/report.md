...
- `report.json`: `game-screen-pick/report@2.2.0`の唯一のmachine-readable正本
...
公開成果物の`annotation.summary`は、選定用のSemantic Annotation Summaryを公開前のcopyで置換したPublication Annotation Summaryです。Scene Display Nameや推定した固有名を連結せず、検証済みのCombat Encounter Kindと画像から直接観測した画面内容だけから有限表現を生成します。戦闘種別を「具体的なプレイ」へ反映するのは画像もgameplayの場合に限り、event画像はeventとして説明します。通常playの移動・待機を断定せず「通常プレイ画面」、人物の有無や会話相手を断定せず「画面内テキストのあるイベント」のように表します。この置換結果を最終selectorの意味重複判定へ戻しません。
...
Semantic Duplicate Groupの代表には`selected[].selection.semantic_group`として決定的なGroup IDとprivacy-safeなbasisを記録します。同じGroupで除外されたnear missには`semantic_duplicate`、blocking selected ID、同じGroup IDとbasisを記録します。basisは`combat_subject_appearance`、`combat_encounter_sequence`、`title_semantics`、`visual_role_similarity`のenumです。各basisは、その根拠を生成した元Groupのmemberまたは先に使われたmemberを除いた2件以上の残余memberだけに記録し、重なった別Groupを経由して無関係なmemberへ拡張しません。`combat_subject_appearance`だけはCombat Encounter Subject Profile間で裏付けられたbody plan・scale・surface・color・traitの有限enum tokenを`evidence`へ必須で記録し、他basisでは`evidence`を出しません。Group内の単発矛盾や`unclear`観測は公開evidenceへ混ぜません。tokenはprefixだけでなくfieldごとの許可値を含む完全enumで検証し、`body_plan:red`のようなカテゴリ不一致も拒否します。対応majorのreaderは、除外理由にかかわらず全selected・near missの既知Semantic Group basisとevidenceを検証します。`semantic_duplicate`についてだけ、同じGroup ID・basis・evidenceの選択代表とblocking selected IDの参照一致も確認します。raw model response、固有名の正誤、比較用自由文は公開しません。
