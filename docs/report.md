...
Semantic Duplicate Groupの代表には`selected[].selection.semantic_group`として決定的なGroup IDとprivacy-safeなbasisを記録します。同じGroupで除外されたnear missには`semantic_duplicate`、blocking selected ID、同じGroup IDとbasisを記録します。basisは`combat_subject_appearance`、`combat_encounter_sequence`、`title_semantics`、`visual_role_similarity`のenumです。各basisは、その根拠を生成した元Groupのmemberにだけ記録し、重なった別Groupを経由して無関係なmemberへ拡張しません。`combat_subject_appearance`だけはGroup memberに共通するbody plan・scale・surface・color・traitの有限enum tokenを`evidence`へ必須で記録し、他basisでは`evidence`を出しません。tokenはprefixだけでなくfieldごとの許可値を含む完全enumで検証し、`body_plan:red`のようなカテゴリ不一致も拒否します。対応majorのreaderは、除外理由にかかわらず全selected・near missの既知Semantic Group basisとevidenceを検証します。`semantic_duplicate`についてだけ、同じGroup ID・basis・evidenceの選択代表とblocking selected IDの参照一致も確認します。raw model response、固有名の正誤、比較用自由文は公開しません。
