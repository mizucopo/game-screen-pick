...
Frontier導入前のcache、またはFrontierが欠ける境界は、後続Candidate Annotationが存在することだけを不足証明として扱いません。Annotationは異なる枚数・Spoiler Sensitivity・Similarity設定のrunでも共有されるため、その推測ではResume Output Invarianceを保証できないからです。未確認境界は一度だけ通常選定してFrontierを確定し、次の中断以降を高速化します。7,000件fixtureの30分予算は、同じ選定意味入力で確定済みのFrontierを持つcurrent-version cacheを対象に検証し、Frontierのないlegacy cacheを勝手に移行しません。
