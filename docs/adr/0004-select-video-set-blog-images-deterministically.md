...
- `combat_subject_appearance` compares deterministic Combat Encounter Subject Profiles rather than requiring every raw frame observation to agree. Each Candidate Moment contributes at most one complete `distinctive` observation. Unique modal body plan, scale, and surface plus colors and traits repeated in at least two Candidate Moments form the profile, so an isolated effect or partial view does not replace it. Profiles match only when their core values agree, their color and trait sets each intersect, and at least one pair of raw candidates supporting the two profiles has Neutral visual similarity of at least 0.80. Profile groups use complete linkage over every Profile pair; a group of three or more Profiles does not additionally require one color or trait shared globally. Video Source, encounter time, Scene Slug, and names are deliberately irrelevant. Generic, unclear, incomplete, and title-frame evidence cannot establish cross-video identity.
- `combat_encounter_sequence` orders every candidate within one Video Source, splits encounters at non-`major` candidates, and then groups chronological `major`-combat Scene Slug runs. A one-candidate slug blip is bridged only when matching slug runs surround it and both adjacent gaps are at most 15 seconds. Each run is one subject by default. It splits only when at least two evidence clusters are each supported by two or more distinct Candidate Moments and their aggregated Profiles are mutually incompatible; sibling frames from one Candidate Moment count once. If any aggregated Profile cannot be formed or any Profile pair remains compatible, the run stays one group. Complete concrete `generic` evidence may corroborate this encounter-local split but never cross-video identity. After a confirmed split, isolated or unclear candidates join the temporally nearest corroborated subject with a stable tie-break and cannot become an uncorroborated extra selection.
...
Overlapping groups are merged deterministically only when an originating group actually contains every member of the connected component. The published basis priority is `combat_subject_appearance`, `combat_encounter_sequence`, `title_semantics`, then `visual_role_similarity`, considering only those component-spanning groups whose public contract is satisfied. If no originating group spans the component, originating groups are processed in the same priority order. Members already claimed by a higher-priority group are removed, and any residual subset with at least two members is retained under the same basis. This never transitively extends a basis through a different group and does not discard lower-priority duplicate constraints among unclaimed members. `combat_subject_appearance` is eligible only when the retained original encounter Profiles cover every member and yield finite evidence tokens; individual noisy or unclear members do not invalidate a corroborated Profile. The original Profile boundaries are not recomputed from flattened subject-group members, so a non-major separator between repeated same-Slug encounters remains effective. Pre-merge Combat Encounter Group edges remain available to the Shortlist boundary check even when they are not published or the published basis becomes `combat_subject_appearance`; otherwise an unannotated non-major boundary could be hidden. A Combat Subject Group publishes the body plan, scale, and surface shared by every Profile plus only globally common color and trait tokens. Globally common color or trait tokens may be absent from a valid pairwise complete-link group. It never publishes a name, model response, or comparison prose.
