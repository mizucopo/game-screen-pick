...
- `combat_subject_appearance` compares only `major` candidates with complete `distinctive` Combat Subject Evidence. Every pair in a group must have equal body plan, scale, and surface, at least one shared color and trait, and Neutral visual similarity of at least 0.80. Video Source, encounter time, Scene Slug, and names are deliberately irrelevant. Generic, unclear, or incomplete evidence cannot create a cross-video group.
- `combat_encounter_sequence` orders every candidate within one Video Source, splits encounters at non-`major` candidates, and then groups chronological `major`-combat Scene Slug runs. A one-candidate slug blip is bridged only when matching slug runs surround it and both adjacent gaps are at most 15 seconds. The same slug after a different major-combat run or a non-major scene starts a different encounter. Within one encounter, identifiable evidence is compatible without the 0.80 Neutral threshold when body plan, scale, and surface match and the color and trait sets each intersect. Concrete `generic` evidence can reveal a conflict in any mutually observed body-plan, scale, surface, color, or trait field, but it cannot establish cross-video identity or a repeated-subject subgroup after a conflict. When one encounter contains multiple clearly different subjects, only compatible repeated `distinctive` subjects use the auxiliary encounter evidence; uncertain subjects are not attached across that distinction.
...
Overlapping groups are merged deterministically only when an originating group actually contains every member of the connected component. The published basis priority is `combat_subject_appearance`, `combat_encounter_sequence`, `title_semantics`, then `visual_role_similarity`, considering only those component-spanning groups whose public contract is satisfied. If no originating group spans the component, originating groups are processed in the same priority order. Members already claimed by a higher-priority group are removed, and any residual subset with at least two members is retained under the same basis. This never transitively extends a basis through a different group and does not discard lower-priority duplicate constraints among unclaimed members. `combat_subject_appearance` is eligible only when at least one finite evidence token is common to every member; otherwise publication falls back only to a lower-priority originating group that spans the same component. Pre-merge Combat Encounter Group edges remain available to the Shortlist boundary check even when they are not published or the published basis becomes `combat_subject_appearance`; otherwise an unannotated non-major boundary could be hidden. A Combat Subject Group publishes only the finite body-plan, scale, surface, color, and trait tokens common to every member; it never publishes a name, model response, or comparison prose.
