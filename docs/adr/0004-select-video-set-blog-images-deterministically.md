...
Overlapping groups are merged deterministically. The published basis priority is `combat_subject_appearance`, `combat_encounter_sequence`, `title_semantics`, then `visual_role_similarity`, considering only bases whose public contract can describe the merged component. `combat_subject_appearance` is eligible only when at least one finite evidence token is common to every member; otherwise publication falls back to the next applicable basis. Pre-merge Combat Encounter Group edges remain available to the Shortlist boundary check even when the published basis becomes `combat_subject_appearance`; otherwise an unannotated non-major boundary could be hidden by the merge. A Combat Subject Group publishes only the finite body-plan, scale, surface, color, and trait tokens common to every member; it never publishes a name, model response, or comparison prose.
