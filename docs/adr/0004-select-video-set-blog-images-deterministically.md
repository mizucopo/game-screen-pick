...
Overlapping groups are merged deterministically only when an originating group actually contains every member of the connected component. The published basis priority is `combat_subject_appearance`, `combat_encounter_sequence`, `title_semantics`, then `visual_role_similarity`, considering only those component-spanning groups whose public contract is satisfied. If no originating group spans the component, non-overlapping originating groups are retained in priority order with their original member boundaries; a basis is never transitively extended through a different group. `combat_subject_appearance` is eligible only when at least one finite evidence token is common to every member; otherwise publication falls back only to a lower-priority originating group that spans the same component. Pre-merge Combat Encounter Group edges remain available to the Shortlist boundary check even when they are not published or the published basis becomes `combat_subject_appearance`; otherwise an unannotated non-major boundary could be hidden. A Combat Subject Group publishes only the finite body-plan, scale, surface, color, and trait tokens common to every member; it never publishes a name, model response, or comparison prose.
