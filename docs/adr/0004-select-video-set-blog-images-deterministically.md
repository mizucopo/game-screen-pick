...
- `combat_encounter_sequence` groups chronological `major`-combat Scene Slug runs within one Video Source. A one-candidate slug blip is bridged only when matching slug runs surround it and both adjacent gaps are at most 15 seconds. The same slug after a different major-combat run starts a different encounter.
