...
- `-n <数値>`, `--num <数値>`: 選択枚数 (デフォルト: 100、多様性を満たせない場合はこれより少なくなることがあります)
- `-s <数値>`, `--similarity <数値>`: 類似度しきい値（0.0〜1.0、デフォルト: 0.72）
  - 値を高くするほど類似画像を許容しやすくなり、指定枚数を満たしやすい一方で、結果は似た画面に寄りやすくなります
  - 値を低くするほど類似画像を厳しく除外し、多様性は上がりやすい一方で、指定枚数より少ない結果になりやすくなります
...
- `--rename`: scene に応じて `gameplay0001.ext` / `event0001.ext` / `other0001.ext` 形式で出力
...
# scene ごとに gameplay0001.ext / event0001.ext 形式へリネームして出力
...
`--similarity` はこの除外判定の基準値で、
**高いほど判定が緩く、低いほど判定が厳しく** なります。

- 例: `--similarity 0.90`
  - かなり似ている画像だけを除外するため、似た構図や近い場面も残りやすくなります
- 例: `--similarity 0.60`
  - 少しでも似ている画像を除外しやすくなるため、バリエーションは増えやすいですが、候補不足で枚数が減りやすくなります
