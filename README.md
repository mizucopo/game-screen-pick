...
- `--no-resume-cache`: 中立解析の再開キャッシュを使わない
...
- `--scene-hint` を変更して再実行した場合、中立解析結果は再利用されますが、scene catalog 作成とOllama分類は新しいヒントに基づいて実行されます
- 古いOllama分類キャッシュは削除されず、scene catalog ごとに別のキャッシュとして保持されます
- 中立解析キャッシュを使わない場合は `--no-resume-cache` を指定します
