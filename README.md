...
- `--ollama-host <URLまたはhost>`: Ollamaホスト。`OLLAMA_HOST` より優先。`192.168.1.31` のようにschemeを省略した場合は `http://` を補完
...
Ollama host の優先順位は `--ollama-host`、`OLLAMA_HOST`、`[ollama].host`、`http://localhost:11434` です。`192.168.1.31` のようにschemeを省略したhostは `http://192.168.1.31` として扱われます。
